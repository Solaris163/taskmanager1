...
{
//    public function behaviors() //поведение ограничивает доступ к странице карточки задачи неавторизованным пользователям
//    {
//        return [
//            'access' => [
...
//    }
    ...
    public function actionIndex() //новый индекс, который сделал по образцу как у преподавателя
    {
        ...
        if (isset(\Yii::$app->request->get()['month'])) //проверяем был ли запрос на на фильтрацию задач по месяцу
        {
            $month = \Yii::$app->request->get()['month']; //получаем из гет запроса месяц для фильтрации задач по месяцу
        }else $month = '0';
        //$month = \Yii::$app->request->get()['month'];
        if ($month === '0') //если месяц равен 0, то передаем в рендер параметр $filter = false
        {
            $filter = false;
        }else $filter = true;
    ...
    public function actionCard($id){ //показывает одну карточку задачи, id приходит сам из get-запроса
        ...
        $users = Users::find()->asArray()->all(); //получаем всех пользователей из базы данных
...
//        );
        ...
        if ($request){ //проверяем, был ли post-запрос на сохранение задачи в базу данных
            if ($canTaskUpdate){
                ...
                echo "У вас недостаточно прав для редактирования задачи. Это может делать только пользователь boss. <br>";
